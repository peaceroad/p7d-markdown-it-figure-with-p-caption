...
- `autoLabelNumberPolicy` is opt-in formatting/scope behavior and does not enable marks. Its separator is `.` (default) or `-`, and scope sources are `frontmatter` and/or top-level `heading` with configured levels.
